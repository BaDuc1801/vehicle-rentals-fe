...
const DetailVehicle = ({ items }) => {
    ...
    return (
        <div className='max-md:flex-col flex max-md:justify-around shadow-md max-md:items-center justify-between md:h-[160px] bg-white rounded-md overflow-hidden w-full m-auto'>
            <img src={items?.img} className='w-[180px] h-full max-md:w-full max-md:h-auto'></img>
            <div className='flex flex-col h-full pl-4 max-md:w-full max-md:pl-5 lg:w-[300px] z-20'>
                <p className='font-semibold text-[17px] max-md:py-5 py-3 max-md:text-[18px]'>{items?.name}</p>
                <div className='text-[14px] md:flex md:justify-around grid grid-cols-2'>
                    <div>
                        ...
                        <p className='flex items-center gap-2  whitespace-nowrap mt-1'><MdMap />{items?.district}, {items?.location}</p>
                    </div>
                    <div>
                        <p className='flex items-center gap-2'><BsFuelPumpFill />{items?.fuel}</p>
                        <p className='flex items-center gap-2 whitespace-nowrap mt-1'><AiFillTool />Sản xuất {items?.year}</p>
                    </div>
                </div>
            </div>
            <div className='flex flex-col justify-around h-full pr-4 w-[180px] max-md:justify-start max-md:w-full max-md:px-5 max-md:pt-5'>
                ...
                <div className='w-full md:flex md:justify-end'>
                    <Button type='primary' className=' max-md:my-5 md:w-[100px] w-full' onClick={onClick}>Chi tiết</Button>
                </div>
            </div>
        </div>
